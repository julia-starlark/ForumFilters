...
public class ForumController {
	...
	@PutMapping("/post")
	public Post updatePost(@RequestBody PostUpdateDto postUdpate, @RequestHeader("Authorization") String token) {
		return forumService.updatePost(postUdpate,token);
	}

	@PutMapping("/post/{id}/likes")
	public boolean addLike(@PathVariable String id, HttpServletRequest request) {
		return forumService.addLike(id, request);
	}
	...
	public Iterable<Post> findPostsByAutor(@PathVariable String author){
		return forumService.findPostsByAutor(author);
	}
