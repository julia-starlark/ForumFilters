...
import telran.ashkelon2018.forum.exceptions.UserExistsException;
...
public class AccountServiceImpl implements AccountService {
	...
	public UserProfileDto addUser(UserRegDto userRegDto, String token) {
		...
		String hashPassword = BCrypt.hashpw(credentials.getPassword(), BCrypt.gensalt()); // 1-what 2-sol
		UserAccount userAccount = UserAccount.builder().login(login).password(hashPassword)
	...
	@Override
	public UserProfileDto updateUser(UserRegDto userRegDto, String token) {
		AccountUserCredentials credentials = accountConfiguration.tokenDecode(token);
	...
	public UserProfileDto removeUser(String login, String token) {
		UserAccount userAccount = repository.findById(login).get();
	...
	public Set<String> addRole(String login, String role, String token) {
		UserAccount userAccountToModify = repository.findById(login).orElse(null);
		userAccountToModify.addRole(role);
		repository.save(userAccountToModify);
		return userAccountToModify.getRoles();
	}
